...
class SoapXmlCreator
{
    ...
    function __construct($tag, $cdata = false)
    {
    ...
    // returns created SoapXmlCreator node with setted TagName and Attributes
    ...
    function encodeValueLight($name, $value)
    {
        ...
        $node = SoapXmlCreator::createTagAttributed($name);
        $name = $node->tag;
        ...
        if (is_object($value) && strtolower(get_class($value)) == "SoapXmlCreator") {
            $node->addChild($value);
        } else if (is_object($value)) {
            $ovars = get_object_vars($value);
            foreach ($ovars as $pn => $pv) {
                $decode = SoapXmlCreator::encodeValueLight($pn, $pv);
    ...
    function addChild($element)
    {
        ...
        if ($element && (get_class($element) == "SoapXmlCreator" || get_class($element) == "SoapXmlCreator")) {
    ...
    {
        return SoapXmlCreator::__createFromDOM($dom->root[0]);
    }

    /* static */
    function __createFromDOM($domNode)
    {
        $result = new SoapXmlCreator($domNode->name);
        ...
        if (is_array($domNode->children)) {
            foreach ($domNode->children as $domChild) {
                $result->addChild(SoapXmlCreator::__createFromDOM($domChild));
            }
        }
