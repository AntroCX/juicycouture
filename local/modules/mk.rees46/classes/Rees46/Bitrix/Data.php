    ...
    public static function getProductPhotoId($item_id) {
        ...
        if(!empty($item['PREVIEW_PICTURE']) || !empty($item['DETAIL_PICTURE'])){
            ...
            if(!empty($item['PREVIEW_PICTURE'])){
                $picture[0] = $picture[1] = $item['PREVIEW_PICTURE'];
                if(!empty($item['DETAIL_PICTURE'])) {
                    $picture[1] = $item['DETAIL_PICTURE'];
                }
            }
        ...
        if($picture[0] == null || $picture[1] == null) {
            // Получаем цену товара или товарного предложения
            if (CCatalogSku::IsExistOffers($item_id)) {
                ...
                if ($ar_res = $res->GetNext()) {
                    ...
                    if (isset($ar_res['IBLOCK_ID']) && $ar_res['IBLOCK_ID']) {
                        ...
                        if(!empty($offers[0]['PREVIEW_PICTURE']['ID']) && $picture[0] == null){
                            $picture[0] = $offers[0]['PREVIEW_PICTURE']['ID'];
                        }
                        if(!empty($offers[0]['DETAIL_PICTURE']['ID']) && $picture[1] == null){
                            $picture[1] = $offers[0]['DETAIL_PICTURE']['ID'];
                        }

                        if($picture[0] == null || $picture[1] == null) {

                            if (is_array($offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"])) {
                                if(!empty($offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"][0])){
                                    $more_photo[0] = $more_photo[1] = $offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"][0];
                                    if(!empty($offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"][1])){
                                        $more_photo[1] = $offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"][1];
                                    }
                                }

                            } elseif (!empty($offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"])) {
                                $more_photo[0] = $more_photo[1] = $offers[0]["PROPERTIES"]["MORE_PHOTO"]["VALUE"];
                            }

                            if($picture[0] == null && !empty($more_photo[0]))
                                $picture[0] = $more_photo[0];
                            if($picture[1] == null && !empty($more_photo[1]))
                                $picture[1] = $more_photo[1];
                        }
