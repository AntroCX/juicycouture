...
class YKassaHandler extends PaySystem\ServiceHandler implements PaySystem\IRefundExtended, PaySystem\IHold
{
    /**
     * Возвращает значение параметра payment_id, вместо orderNumber в оригинале
     *
    ...
    private function processNoticeAction(Payment $payment, Request $request)
    {
        ...
        if ($this->isCorrectSum($payment, $request))
        {
            ...
            if ($this->getBusinessValue($payment, 'PS_CHANGE_STATUS_PAY') == 'Y'){
                ...
                // проставляем оплату в OCS
                $orderId = (int)$request->get('orderNumber');
