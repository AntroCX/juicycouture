...
/** профиль REES46 ? */
$REES46 = 0;
if(strpos($SETUP_FILE_NAME, "rees46") !== false){
    $REES46 = 1;
}
...
/** беспл. доставка */
define("MIN_PRICE_FREE_DELIVERY", 10000);

/** доставка Москва и Питер */
/** JIRA ALL-9 */
$arCity = array('moscow', 'piter');
$deliveryTarifIblockId = 16;

$arDeliveryParams = array();
$dbRes = CIBlockElement::GetList(
    array(),
    array('IBLOCK_ID' => $deliveryTarifIblockId, 'ACTIVE' => 'Y',
        array('LOGIC' => "OR", array('NAME' => 'москва', 'PROPERTY_address_region'=> 'московская%'), array('NAME' => 'санкт-петербург', 'PROPERTY_address_region'=> 'ленинградская%'))),
    false,
    false,
    array('NAME', 'ID', 'PROPERTY_address_region', 'PROPERTY_value', 'PROPERTY_terms')
);
while ($arRes = $dbRes->Fetch())
{
    if(strpos(tolower($arRes["NAME"]), 'москва') !== false){
        $arDeliveryParams['moscow'] = array(
            "price" => $arRes["PROPERTY_VALUE_VALUE"],
            "days" => "1",
            "order-before" => 16,
            "city-name" => $arRes["NAME"]
        );
    }
    else if(strpos(tolower($arRes["NAME"]), 'петербург') !== false){
        $arDeliveryParams['piter'] = array(
            "price" => $arRes["PROPERTY_VALUE_VALUE"],
            "days" => "2",
            "order-before" => 16,
            "city-name" => $arRes["NAME"]
...
foreach($arCity as $city){
    if(strpos($SETUP_FILE_NAME, $city) !== false){
        $arDeliveryParams = $arDeliveryParams[$city];
        break;
    }
}

define("SIZES_SHOES_CODE", "SIZES_SHOES");
define("SIZES_CLOTHES_CODE", "SIZES_CLOTHES");
define("SIZES_RINGS_CODE", "SIZES_RINGS");
global $SIZE_PROP_CODES;
$SIZE_PROP_CODES = array(
    SIZES_CLOTHES_CODE,
    SIZES_SHOES_CODE,
    SIZES_RINGS_CODE
);

/** таблица соответствия размеров INT(BRAND)->RU */
global $arShoesSize, $arClothesSize, $arClothesBrandSize;
$arShoesSize = [
    "5" =>	"35",
    "5.5" => "35.5",
    "6" => "36",
...
$arYandexFields = array(
	...
	'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date', 'group_id'
);
...
{
	global $iblockServerName, $arShoesSize, $arClothesSize, $arClothesBrandSize, $SIZE_PROP_CODES;
	...
	if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
	{
		...
		switch ($arProperties[$PROPERTY]['PROPERTY_TYPE']) {
            ...
            case Iblock\PropertyTable::TYPE_LIST:
                /** размерная сетка */
                if (in_array($PROPERTY_CODE, $SIZE_PROP_CODES)) {
                    if (!empty($arProperty['VALUE']) || $arProperty['VALUE'] == 0) {
                        $f_size_int = true;
                        if ($PROPERTY_CODE == SIZES_SHOES_CODE) {
                            if (array_key_exists($arProperty['VALUE'], $arShoesSize)) {
                                $value = $arShoesSize[$arProperty['VALUE']];
                                $f_size_int = false;
                            }
                        } else {
                            if (array_key_exists($arProperty['VALUE'], $arClothesBrandSize)) {
                                $value = $arClothesBrandSize[$arProperty['VALUE']];
                                $f_size_int = false;
                            }
                            /*
                            else if (array_key_exists($arProperty['VALUE'], $arClothesSize)) {
                                $value = $arClothesSize[(string)trim($arProperty['VALUE'])];
                                $f_size_int = false;
                            }
                            */
                        }
		...
		if ($bParam)
		{
			...
			{
                if (in_array($arProperty["CODE"], $SIZE_PROP_CODES) ) {
                    $arProperty['NAME'] = "Размер";
                    if($f_size_int)
                        $f_size_value = "INT";
                    else
                        $f_size_value = "RU";
                    $strProperty .= '<param name="' . yandex_text2xml($arProperty['NAME'], true) . '"' . ' unit="' . yandex_text2xml($f_size_value, true) . '">' .
...
if (empty($arRunErrors))
{
	...
	if (!$bAllSections)
	{
		...
		{
			$sectionIterator = CIBlockSection::GetNavChain($IBLOCK_ID, $arSections[$i], array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'ACTIVE'));
			$curLEFT_MARGIN = 0;
			$curRIGHT_MARGIN = 0;
			while ($section = $sectionIterator->Fetch())
			{
				if($section['ACTIVE'] == 'N') continue;
			...
			$sectionIterator = CIBlockSection::GetList(array("LEFT_MARGIN"=>"ASC"), $filter, false, array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'ACTIVE'));
			while ($section = $sectionIterator->Fetch())
			{
				$section["ID"] = (int)$section["ID"];
				$section["IBLOCK_SECTION_ID"] = (int)$section["IBLOCK_SECTION_ID"];
	...
	} elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
	{
		$arOfferSelect = array(
			...
			"PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE",  "DETAIL_PICTURE", "DETAIL_PAGE_URL",
			"CATALOG_AVAILABLE", "CATALOG_TYPE",
            "PROPERTY_CML2_LINK.NAME", "PROPERTY_CML2_LINK.ID", "PROPERTY_".PICTURE_PROP_CODE,
            "CATALOG_GROUP_".SALE_PRICE_TYPE, "CATALOG_GROUP_".BASE_PRICE_TYPE
		);
		...
		if ($filterAvailable && !$REES46) // все активные для REES46
		...
		while ($obItem = $rsItems->GetNextElement())
		{
            ...
            /** experimental - удаляем кириллицу из названия */
            //$model = preg_replace("/[а-яё]/ius", "", $arItem["NAME"]);
            ...
            $tryDescr = preg_replace("/\r?\n?/s", "", trim($arItem["~PREVIEW_TEXT"]));
            $typeDescr = "PREVIEW";
            if(empty($tryDescr)){
                $tryDescr = preg_replace("/\r?\n?/s", "", trim($arItem["~DETAIL_TEXT"]));
                $typeDescr = "DETAIL";
            }

			$arItem['YANDEX_DESCR'] = yandex_text2xml(TruncateText(
							($arItem[$typeDescr."_TEXT_TYPE"]=="html"?
							strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $tryDescr)) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $tryDescr)),
							1000), true);
			...
			if ($arItem['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU)
			{
				...
				else
				{
					while ($obOfferItem = $rsOfferItems->GetNextElement())
					{
						...
						if($arOfferId == $arOfferItem["ID"])
						    continue;
						$arOfferId = $arOfferItem["ID"];
						...
						{
                            /** скидка из типа цены */
                            $basePrice = intval($arOfferItem["CATALOG_PRICE_".BASE_PRICE_TYPE]);
                            $salePrice = intval($arOfferItem["CATALOG_PRICE_".SALE_PRICE_TYPE]);
						...
						$strOfferYandex = '';
						$strOfferYandex .= '<offer id="'.$arOfferItem["ID"].'"'.$productFormat.' available="'.$arOfferItem['YANDEX_AVAILABLE'].'"'.' group_id="'.yandex_text2xml(trim($arOfferItem["~PROPERTY_CML2_LINK_ID"]), true).'">'."\n";
						$offersCnt++;
						...
						/** обнуляем доставку */
                        $IS_FREE_DELIVERY = 0;
						if($minPrice >= MIN_PRICE_FREE_DELIVERY)
                            $IS_FREE_DELIVERY = 1;
						...
						if ($arOfferItem["DETAIL_PICTURE"] > 0 || $arOfferItem["PREVIEW_PICTURE"] > 0)
						{
							$pictNo = ($arOfferItem["PREVIEW_PICTURE"] > 0 ? $arOfferItem["PREVIEW_PICTURE"] : $arOfferItem["DETAIL_PICTURE"]);
                        ...
                        /** картинка товара из свойства */
						if(empty($strFile) && !empty($arOfferItem["PROPERTY_".PICTURE_PROP_CODE."_VALUE"])){

                            if(is_array($arOfferItem["PROPERTY_".PICTURE_PROP_CODE."_VALUE"]))
                                $pic_id = $arOfferItem["PROPERTY_".PICTURE_PROP_CODE."_VALUE"][0];
                            else
                                $pic_id = $arOfferItem["PROPERTY_".PICTURE_PROP_CODE."_VALUE"];

                            if ($ar_file = CFile::GetFileArray($pic_id))
                            {
                                if (substr($ar_file["SRC"], 0, 1) == "/")
                                    $strFile = $usedProtocol.$ar_iblock['SERVER_NAME'].CHTTP::urnEncode($ar_file['SRC'], 'utf-8');
                                else
                        ...
                        if(!$REES46) {
                            ...
                            $rsCatalog = \CCatalogStoreProduct::GetList(array(), $arFilter, false, false, $arFields);
                            $arCatalog = $rsCatalog->Fetch();
                            if ($arCatalog['ID']) {
                                $propYN = intval($arCatalog["AMOUNT"]) > 0 ? "true" : "false";
                            }
                            $strOfferYandex .= "<store>".yandex_text2xml($propYN, true)."</store>\n";
                            $strOfferYandex .= "<pickup>".yandex_text2xml($pickupOff ? 'false' : $propYN, true)."</pickup>\n";
                            $strOfferYandex .= "<delivery>".yandex_text2xml("true", true)."</delivery>\n";

                            /** доставка */
                            if (!empty($arDeliveryParams)) {
                                $delivery_price = $IS_FREE_DELIVERY ? 0 : $arDeliveryParams["price"];
                                $strOfferYandex .= "<delivery-options>\n";
                                $strOfferYandex .= "<option cost=\"".yandex_text2xml($delivery_price, true)."\" days=\"".yandex_text2xml(
                                        $arDeliveryParams["days"],
                                        true
                                    )."\" order-before=\"".yandex_text2xml($arDeliveryParams["order-before"], true)."\" />\n";
                                $strOfferYandex .= "</delivery-options>\n";
                            }

                            /** cpa */
                            $strOfferYandex .= "<cpa>".yandex_text2xml("1", true)."</cpa>\n";

                            /** sale notes */
                            $strOfferYandex .= "<sales_notes>".yandex_text2xml("не более 10 товаров в заказе", true)."</sales_notes>\n";
                        }
                        ...
                        $strOfferYandex .= "<param name=\"".yandex_text2xml("Пол", true)."\">".yandex_text2xml($sex, true)."</param>\n";
                        $strOfferYandex .= "<param name=\"".yandex_text2xml("Возраст", true)."\">".yandex_text2xml("Взрослый", true)."</param>\n";
						...
						foreach ($arYandexFields as $key)
						{
							...
							{
                                case 'model':
                                    /** принудительно меняем на назвние */
                                    $strOfferYandex .= "<".$key.">".yandex_text2xml($arItem["~NAME"], true)."</".$key.">\n";
                                    break;
                                case 'name':
									if ($yandexFormat == 'vendor.model' || $yandexFormat == 'artist.title')
										continue;

									$strOfferYandex .= "<name>".yandex_text2xml(trim($arOfferItem["~PROPERTY_CML2_LINK_NAME"]), true)."</name>\n";
									break;
								case 'description':
									$strOfferYandex .= "<description>";

                                    $tryDescr = preg_replace("/\r?\n?/s", "", trim($arOfferItem["~PREVIEW_TEXT"]));
                                    $typeDescr = "PREVIEW";
                                    if(empty($tryDescr)){
                                        $tryDescr = preg_replace("/\r?\n?/s", "", trim($arOfferItem["~DETAIL_TEXT"]));
                                        $typeDescr = "DETAIL";
                                    }

									if (strlen($tryDescr) <= 0)
									{
										$strOfferYandex .= $arItem['YANDEX_DESCR'];
									}
									else
									{
										$strOfferYandex .= yandex_text2xml(TruncateText(
											($arOfferItem[$typeDescr."_TEXT_TYPE"] == "html" ?
												strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $previewText)) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $tryDescr)),
											1000),
											true);
									}
									$strOfferYandex .= "</description>\n";
    ...
    if(!empty($arDeliveryParams)){
        ...
        fwrite($fp, "<option cost=\"".yandex_text2xml($arDeliveryParams["price"], true)."\" days=\"".yandex_text2xml($arDeliveryParams["days"], true)."\" order-before=\"".yandex_text2xml($arDeliveryParams["order-before"], true)."\" />\n");
        fwrite($fp,"</delivery-options>\n");
    }
...
if(empty($arRunErrors)){
    $msg = "Яндекс фид (для региона {$arDeliveryParams['city-name']}) сгенерирован.\r\n";
    $msg .= "Выгружено торг. предложений: ".$offersCnt;
}
else{
    $msg = "При генерации Яндекс фида (для региона {$arDeliveryParams['city-name']}) произошла ошибка.\r\n";
    $msg .= implode('\r\n',$arRunErrors);
}
send_message($msg);

function send_message($msg){
    global $sendEmail, $iblockServerName;
    if(!$sendEmail)
        return;
    $receivers = "vasiliev@jamilco.ru, AlFeisal@jamilco.ru";
    $body = date("d.m.Y  H:i:s").".\r\n".$msg;
    $subject = $iblockServerName.". Yandex feed message.";
