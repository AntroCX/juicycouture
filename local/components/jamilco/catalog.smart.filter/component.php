...
global ${$FILTER_NAME};
if (!is_array(${$FILTER_NAME})) ${$FILTER_NAME} = [];
...
if ($this->StartResultCache(false, 'v5'.($arParams["CACHE_GROUPS"] ? $USER->GetGroups() : false))) {
    ...
    if (!empty($arResult["ITEMS"])) {
        if ($this->facet->isValid() && 0) {
            ...
            if ($arElementFilter['OFFERS']) {
                $arElementFilter['=ID'] = CIblockElement::SubQuery('PROPERTY_CML2_LINK', $arElementFilter['OFFERS']);
            }
            ...
            if (!empty($arElements) && $this->SKU_IBLOCK_ID && $arResult["SKU_PROPERTY_COUNT"] > 0) {
                ...
                if (${$FILTER_NAME}['OFFERS']) $arSkuFilter = array_merge($arSkuFilter, ${$FILTER_NAME}['OFFERS']);

                $rsElements = CIBlockElement::GetPropertyValues(
                    $this->SKU_IBLOCK_ID,
                    $arSkuFilter,
                    false,
                    array('ID' => $this->arResult["SKU_PROPERTY_ID_LIST"])
                );
                        ...
                        if (isset($arSku[$PID]) && $arSku[$this->SKU_PROPERTY_ID] > 0) {
                            if (is_array($arSku[$PID])) {
                                foreach ($arSku[$PID] as $value) {
                                    if (!in_array($value, $arElements[$arSku[$this->SKU_PROPERTY_ID]][$PID])) {
                                        $arElements[$arSku[$this->SKU_PROPERTY_ID]][$PID][] = $value;
                                    }
                                }
                            } else {
                                if (!in_array($arSku[$PID], $arElements[$arSku[$this->SKU_PROPERTY_ID]][$PID])) {
            ...
            CTimeZone::Disable();
            $uniqTest = array();
            foreach ($arElements as $elemId => $arElement) {
                $propertyValues = $propertyEmptyValuesCombination;
                $uniqStr = '';
                foreach ($arResult["ITEMS"] as $PID => $arItem) {
                    if (is_array($arElement[$PID])) {
                        foreach ($arElement[$PID] as $value) {
                            if ($key = $this->fillItemValues($arItem, $value)) {
                                $arOne = $arItem["VALUES"][$key];
                                $propertyValues[$PID][$key] = $arOne["HTML_VALUE_ALT"].'-'.$arOne["VALUE"];
                                $uniqStr .= '|'.$key.'|'.$propertyValues[$PID][$key];
                            }
                        }
                    } elseif ($arElement[$PID] !== false) {
                        if ($key = $this->fillItemValues($arItem, $arElement[$PID])) {
                            $arOne = $arItem["VALUES"][$key];
                            $propertyValues[$PID][$key] = $arOne["HTML_VALUE_ALT"].'-'.$arOne["VALUE"];
                            $uniqStr .= '|'.$key.'|'.$propertyValues[$PID][$key];
                        }
                    ...
                    $arResult["ITEMS"][$PID] = $arItem;
                }

                $uniqCheck = md5($uniqStr);
                if (isset($uniqTest[$uniqCheck])) continue;
            ...
            if ($arItem["PROPERTY_TYPE"] == "N") {
                ...
                if ($arResult["FACET_FILTER"] && strlen($_CHECK[$ar["CONTROL_NAME"]]) > 0) {
                    if ($key == "MIN") {
                        ...
                        $this->facet->addDatetimePropertyFilter(
                            $PID,
                            "<=",
                ...
                $arResult["ITEMS"][$PID]["DISPLAY_EXPANDED"] = "Y";
                $allCHECKED[$PID][$ar['HTML_VALUE_ALT'].'-'.$ar["VALUE"]] = true;
    ...
    if ($arResult["FACET_FILTER"]) {
        ...
        foreach ($arResult["ITEMS"] as $PID => &$arItem) {
            if ($arItem["PROPERTY_TYPE"] != "N" && !isset($arItem["PRICE"])) {
                ...
                foreach ($arItem["VALUES"] as $key => &$arValue) {
                    $found = false;
                    $valueCheck = $arValue['HTML_VALUE_ALT'].'-'.$arValue['VALUE'];
                    if (isset($index[$PID][$valueCheck])) {
                        //Check if there are any combinations exists
                        foreach ($index[$PID][$valueCheck] as $id => $combination) {
...
// only return filter
if ($arParams['RETURN_FILTER'] == 'Y') return ${$FILTER_NAME};
...
if ($arResult["JS_FILTER_PARAMS"]["SEF_SET_FILTER_URL"]) {
    ...
    $arResult["FILTER_AJAX_URL"] = htmlspecialcharsbx(
        CHTTP::urlAddParams(
            $arResult["FILTER_URL"],
            array(
    ...
    $arResult["FILTER_URL"] = htmlspecialcharsbx(
        CHTTP::urlAddParams(
            $clearURL,
            $paramsToAdd,
            array(
