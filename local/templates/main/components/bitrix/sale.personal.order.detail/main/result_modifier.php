...
foreach ($arResult['BASKET'] as $arBasket) {
    $arProductIDs[] = $arBasket['PARENT']['ID'];
}


if(count($arProductIDs) > 0) {
    $arProductsName = array();

    $rsProducts = \CIBlockElement::GetList(array(), array('IBLOCK_ID' => 1, 'ID' => $arProductIDs));
    while ($arrProducts = $rsProducts->GetNext()) {
        $arProductsName[$arrProducts['ID']] = array(
            'NAME' => $arrProducts['NAME'],
            'URL' => $arrProducts['DETAIL_PAGE_URL']
        );
    }


    foreach ($arResult['BASKET'] as $id => $arBasket) {
        $arResult['BASKET'][$id]['PRODUCT_NAME'] = $arProductsName[$arBasket['PARENT']['ID']]['NAME'];
        $arResult['BASKET'][$id]['URL'] = $arProductsName[$arBasket['PARENT']['ID']]['URL'];
    }
}

$order = \Bitrix\Sale\Order::load($arResult["ACCOUNT_NUMBER"]);
$propertyCollection = $order->getPropertyCollection();
$arResult['TK_NAME'] = $propertyCollection->getItemByOrderPropertyId(ORDER_PROP_TK_NAME)->getValue();
$arResult['TK_NUM'] = $propertyCollection->getItemByOrderPropertyId(ORDER_PROP_TK_NUM)->getValue();
?>