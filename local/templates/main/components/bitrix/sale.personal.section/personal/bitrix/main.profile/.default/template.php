...
<div class="bx_profile">
	...
	<form method="post" name="form1" action="<?=$APPLICATION->GetCurUri()?>" enctype="multipart/form-data" role="form">
		...
		<p class="main-profile-form-buttons-block col-sm-9 col-md-offset-3">
			<input type="submit" name="save" class="btn btn-themes btn-default btn-md main-profile-submit btn-brown transition" value="<?=(($arResult["ID"]>0) ? Loc::getMessage("MAIN_SAVE") : Loc::getMessage("MAIN_ADD"))?>">
<?/*?>			&nbsp;
			<input type="submit" class="btn btn-themes btn-default btn-md btn-brown transition"  name="reset" value="<?echo GetMessage("MAIN_RESET")?>">
<?*/?>
		</p>
	</form>
