            ...
            if($arParams['IS_SALE'] === 'Y'){
                if((int)$arOffer['PRICES']['SALE']['VALUE'] > 0 && ((int)$arOffer['PRICES']['BASE']['VALUE'] > (int)$arOffer['PRICES']['SALE']['VALUE'])) {
                    $arResult['ITEMS'][$id]['SKU_ITEMS'][$arOffer['PROPERTIES']['COLOR']['VALUE']] = array(
                        ...
                        'PRICE_VALUE'          => $arOffer['PRICES']['BASE']['VALUE'],
                        'PRICE_DISCOUNT_VALUE' => $arOffer['PRICES']['SALE']['VALUE'],
...
/** Если нет иерархии категорий, нужно ее составить. Ее может не быть в разделе Sale или Поиск */
...
foreach ($arResult['ITEMS'] as $key => $arItem) {
    ...
    foreach ($arItem['OFFERS'] as $arOffer) {
        ...
        $variations[] = array(
            ...
            'size'          => $size,
            'unitPrice'     => (float)$arOffer['CATALOG_PRICE_1'],
            'unitSalePrice' => (float)$arOffer['CATALOG_PRICE_2'] ?: (float)$arOffer['CATALOG_PRICE_1'],
        );
    }
    ...
    $ddlItems[] = [
         ...
         */
        'stock'                => (int)$arItem['CATALOG_QUANTITY'], // доступное количество товара
...
$parentCategory = [];
foreach ($arResult['PATH'] as $element) {
    $parentCategory[] = [
        'id' => $element['ID'],
        'name' => $element['NAME']
    ];
...
$childCategory = [];
if ($arResult['ID'] > 0) {
    $rsSection = \CIBlockSection::GetTreeList(['SECTION_ID' => $arResult['ID'], 'ACTIVE' => 'Y'], ['ID', 'NAME']);
...
foreach ($arResult['ITEMS'] as $key => $arItem) {
    ...
    $products[] = [
        'pid' => $arItem['ID'],
        'pname' => $arItem['NAME'],
        'url' => $adspire->formatURL($arItem['DETAIL_PAGE_URL']),
        'picture' => $adspire->formatURL($arItem['SKU_ITEMS'][$colorCode]['FIRST_PICTURE']['src']),
        'price' => (float)$arItem['OFFERS'][$firstOfferId]['PRICES']['SALE']['DISCOUNT_VALUE'] ?: (float)$arItem['MIN_PRICE']['DISCOUNT_VALUE'],
        'currency' => 'RUB',
...
if (is_object($cp)) {
    ...
    /** DigitalDataLayer start */
    $cp->arResult['DDL_SECTION_PROPERTIES'] = [
        'listId'      => 'main',
        'listName'    => 'category',
        'categoryId'  => (string)$arResult['ID'],
        'category'    => array_column($arResult['PATH'] ?: [], 'NAME'),
        'items'       => $ddlItems,
        'resultCount' => (int)$arResult['NAV_RESULT']->NavRecordCount,
        'pagesCount'  => (int)$arResult['NAV_RESULT']->NavPageCount,
        'currentPage' => (int)($arResult['NAV_RESULT']->NavRecordCount > 0 ? $arResult['NAV_RESULT']->NavPageNomer : 0),
        'sortBy'      => 'по умолчанию',
    ];
    /** DigitalDataLayer end */
    ...
    $cp->SetResultCacheKeys(['DDL_SECTION_PROPERTIES', 'ADSPIRE_PROPERTIES']);
}
...
foreach($arResult["ITEMS"] as $itemId => $arItem){
    ...
    foreach($arItem["OFFERS"] as $arOffer_){
        if(!empty($arOffer_["PROPERTIES"]["SIZES_CLOTHES"]["VALUE"])){
            $sizePropCode = "SIZES_CLOTHES";
            break;
        }
    }

    if(empty($sizePropCode)){
        foreach($arItem["OFFERS"] as $arOffer_){
            if(!empty($arOffer_["PROPERTIES"]["SIZES_SHOES"]["VALUE"])){
                $sizePropCode = "SIZES_SHOES";
                break;
            }
        }
    }
    ...
    if(empty($sizePropCode)){
        foreach($arItem["OFFERS"] as $arOffer_){
            if(!empty($arOffer_["PROPERTIES"]["SIZES_RINGS"]["VALUE"])){
                $sizePropCode = "SIZES_RINGS";
                break;
            }
        }
    }
    ...
    if($sizePropCode){
        ...
        foreach($arItem["OFFERS"] as $arOffer){
            if($arOffer["CATALOG_QUANTITY"] < 1)
                continue;
            $arSizes[$arOffer["ID"]] = $arOffer["PROPERTIES"][$sizePropCode]["VALUE"];
        }

        if(!empty($arSizes)){
            $first = array_keys($arResult["ITEMS"][$itemId]["SKU_ITEMS"])[0];
            $arResult["ITEMS"][$itemId]["SKU_ITEMS"][$first]["SIZES"] = $arSizes;
        }

    }
}

/** Получить привязку елементов к разделам */

foreach($arResult["ITEMS"] as  &$arItem){
    $res = CIBlockElement::GetElementGroups($arItem['ID']);
    $arGroups = array();
    while($ob = $res->Fetch())
        $arItem['IBLOCK_SECTION_ID_ALL'][] = $ob["ID"];
}
unset($arItem);
